...
function initPortalLinks() {
  ...
  const portalUrl = isLocal ? 'http://localhost:3000' : 'https://os.kapateconsultancy.com';

  document.querySelectorAll('a').forEach(link => {
    if (link.textContent.trim() === 'Kapate OS') {
      link.href = portalUrl;
      link.target = '_blank';
...
function initContactForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (isValid) {
      ...
      try {
        const response = await fetch('http://localhost:8000/api/v1/public/leads', {
          method: 'POST',
