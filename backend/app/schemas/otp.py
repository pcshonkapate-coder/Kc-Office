...
class OTPRequestPayload(BaseModel):
    identifier: EmailStr = Field(..., description="Employee work email address", examples=["admin@kapateconsultancy.in"])
...
class OTPVerifyPayload(BaseModel):
    identifier: EmailStr = Field(..., description="Employee work email address", examples=["admin@kapateconsultancy.in"])
    otp_code: str = Field(..., min_length=6, max_length=6, pattern=r"^\d{6}$", description="6-digit OTP verification code", examples=["123456"])
